...
#include <stdlib.h>
#include <string.h>
#include <time.h> // Necessário para o controlo de tempo
...
void cifrar_ficheiro() {
    ...
    if (gmp_fscanf(f_in, "%Zd", m) == 1) {
        ...
        printf("Tempo de execução da cifra: %f segundos\n", cpu_time_used);
    } else {
        printf("Erro: O ficheiro deve conter um número.\n");
    }
...
void decifrar_ficheiro() {
    ...
    if (gmp_fscanf(f_in, "%Zd", c) == 1) {
        ...
        printf("Tempo de execução da decifra: %f segundos\n", cpu_time_used);
    }
...
int main() {
    int opcao;
    do {
        printf("\n=== Exercicio 21: RSA com Controlo de Tempo ===\n");
        printf("1. Cifrar Ficheiro\n");
        printf("2. Decifrar Ficheiro\n");
        printf("0. Sair\n");
        printf("Opcao: ");
